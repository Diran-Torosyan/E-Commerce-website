    ...
    private int getLastCustomerId() {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("E-Commerce-website/E-Commerce-website/mavenConversion/my-app/src/main/resources/customer.txt")))) {
            ...
            while ((line = br.readLine()) != null) {
                try {
                    // Assuming the ID is always the first line in each customer entry
                    lastId = Integer.parseInt(line.trim());
                    
                    // Skip the remaining lines of the current customer entry (assuming 8 more lines after the ID line)
                    for (int i = 0; i < 8; i++) {
