...
d) CreatePages is the central class of an e-commerce application. It creates
 ...
 * 
 * <p>The CreatePages class extends JFrame class in the Java Swing library to access its components. It formats an
...
public class CreatePages extends JFrame {
    ...
    private void filterProducts(Map<Integer, Item> productsToFilter) {
        if (menFilter.isSelected()) {
            productsToFilter.values().removeIf(item -> !item.getCategory().equalsIgnoreCase("Men's"));
        }

        if (womenFilter.isSelected()) {
            productsToFilter.values().removeIf(item -> !item.getCategory().equalsIgnoreCase("Women's"));
        }
