...
public class CreatePages extends JFrame {
    ...
    public CreatePages() {
        ...
        homeDisplay();
        itemDisplay(products); // Initialize the item display with all products
    }
    ...
    void itemDisplay(Map<String, Product> productsToDisplay) {
        ...
        for (Product product : productsToDisplay.values()) {
            try {
                ...
                BufferedImage img = ImageIO.read(getClass().getResourceAsStream(product.getImagePath()));
                if (img == null) {
    ...
    private void sendEmailReceipt() {
        ...
        String host = "sandbox.smtp.mailtrap.io"; // Mailtrap SMTP server
        final String username = "3fd865f7e6364f"; // Mailtrap SMTP username
        final String password = "ff57e41eb72590"; // Mailtrap SMTP password
